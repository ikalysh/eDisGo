...
def import_pypower_grid(ppc, edisgo_obj):
    """
    ...
    grid = PyPSANetwork()
    grid.import_from_pypower_ppc(ppc)

    # check v_nom to make sure grid is MV grid with optionally underlying LV
    # grids

    # buses_df: assign mv_grid_id, assign lv_grid_id to LV grids

    # keep slack and change in pypsa io

    # generators_df: add columns weather_cell_id and subtype

    # loads_df: add columns 'peak_load', 'sector', 'annual_consumption'
    # @Daniel: p_set als peak_load setzen?

    # transformers_df: add type_info column with transformer size as name

    # lines_df: 'length' checken ob eine gegeben ist, ansonsten warnung
    # ausgeben; add 'type_info' and 'kind' column
    # @Daniel: kind auf cable setzen?

    # write dataframes to edisgo_obj

    # check voltage
    if (grid.buses.v_nom > 20).all():
        raise AttributeError(
            "eDisGo is only suited for medium and low voltage grids but "
            "provided pypower grid does not contain any medium or low voltage"
            "grids."
        )
    # buses_df
